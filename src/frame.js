...
AFRAME.registerComponent("frame", {
  ...
  update(oldData) {
    ...
    if (this.data.name && !this.title)
    {
      ...
      title.setAttribute('position', `${- width / 2 + 0.055 + this.center.x} ${height / 2 + 0.055 + this.center.y} ${this.center.z}`)
      title.setAttribute('text', `color: #FFF; width: ${width / 2}; align: left; anchor: left; value: ${this.data.name}; wrapCount: 20; zOffset: 0.005; font: ${require('./assets/title-font.woff')}`)
      // title.setAttribute('text', 'font', )
      title.setAttribute('troika-text', 'outlineBlur', 0.03)
      title.setAttribute('troika-text', 'outlineColor', '#0b0804')
      title.setAttribute('troika-text', 'outlineWidth', 0.030)
